        ...
        <div className="text-white py-16 md:py-24 bg-cover bg-center bg-black/90 bg-[url('/images/LeDocBenefitsAxodian.webp')]">
            <div className="container mx-auto">
                <div className="text-center mb-12">
                    <h1 className="text-white leading-tight">Why Axodian?</h1>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                    {ValuesData.map((value, index) => (
                        <div key={value.id} className={`relative flex justify-center py-6 ${index === 1 ? "border-x-2 border-dashed border-white/20" : ""}`}>
                            <Card className="border-none shadow-none bg-transparent text-white max-w-xs">
                                <CardContent className="p-0">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <h5 className="leading-tight">{value.title}</h5>
                                    </div>

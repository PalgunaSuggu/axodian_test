...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const blogResponse = await fetch(
      `${api_url}/articles?sort[0]=title:asc&filters[slug][$eq]=${slug}&status=published&locale[0]=en&populate=*`
    );
    ...
    const formattedBlog = {
      ...
      coverImage: blog.cover?.formats?.large?.url || blog.cover?.url || 'https://www.axodian.com/images/axodian-logo-footer.webp',
      contentImage: blog.cover?.formats?.large?.url || blog.cover?.url || 'https://www.axodian.com/images/axodian-logo-footer.webp',
    ...
    const relatedResponse = await fetch(
      `${api_url}/articles?filters[category][Category][$eq]=${category}&filters[id][$ne]=${blog.id}&populate=*&pagination[limit]=3`
    );
    ...
    const fetchAllRelatedBlogs = async () => {
      try {
        ...
        const response = await fetch(
          `${api_url}/articles?filters[category][Category][$eq]=${blog.category}&filters[id][$ne]=${blog.id}&populate=*`
        );
          ...
          <div className="prose max-w-none">
            <ReactMarkdown
              ...
              components={{
                ...
                img: ({ src, alt }) => {
                  ...
                  return (
                    <div className="my-4 flex justify-center">
                      <Image
                        src={src.startsWith('http') ? src : `${strapi_url}${src}`}
                ...
                blockquote: ({ children }) => (
                  <blockquote className="border-l-4 border-solid border-blue-500 pl-4 italic text-gray-600 my-4">
                    {children}
                  </blockquote>
                ),
                code: ({ children }) => (
                ...
                <Card key={relatedBlog.id} className="overflow-hidden border border-solid border-gray-200 shadow-none bg-white rounded-lg">
                  {relatedBlog.image && (
                    <div className="relative h-[240px]">
                      <Image
                        src={relatedBlog.image ? (relatedBlog.image.startsWith('http') ? relatedBlog.image : `https://strapi.leremitt.com${relatedBlog.image}`) : "/images/Palguna.webp"}
